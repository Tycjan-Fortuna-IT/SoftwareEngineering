...
use Tests\Unit\CommentControllerTest\CommentResourceValidator;
use Tests\Validators\LinksValidator;
use Tests\Validators\MetaValidator;

class CommentControllerIndexTest extends APIUnitTestCase
{
    public function test_comments_index_returns_paginated_data()
    {
        $this->actingAs($this->get_random_user())
            ...
            ->assertJson(function (AssertableJson $json) {
				$json->has('data', function (AssertableJson $data) {
					$data->each(function (AssertableJson $comment) {
    ...
    public function test_comments_index_returns_unpaginated_data()
    {
        $this->actingAs($this->get_random_user())
            ->get('/api/comments?paginate=false')
            ->assertStatus(Response::HTTP_OK)
            ->assertJson(function (AssertableJson $json) {
				$json->has('data', function (AssertableJson $data) {
					$data->each(function (AssertableJson $comment) {
    ...
    public function test_comments_index_returns_correct_amount_of_items_with_custom_per_page_and_page()
    {
        $response = $this->actingAs($this->get_random_user())
            ->get('/api/comments?per_page=5&page=2')
            ->assertStatus(Response::HTTP_OK)
            ->assertJson(function (AssertableJson $json) {
                $json->has('data', function (AssertableJson $data) {
                    $data->each(function (AssertableJson $comment) {
